...
#define THREAD_INF_LOG(fmt, ...)    printf("[THREAD-I]" fmt "\n", ##__VA_ARGS__);
#define THREAD_DBG_LOG(fmt, ...)    printf("[THREAD-D]" fmt "\n", ##__VA_ARGS__);
#define THREAD_WRN_LOG(fmt, ...)    printf("[THREAD-W]" fmt "\n", ##__VA_ARGS__);
#define THREAD_ERR_LOG(fmt, ...)    printf("[THREAD-E]" fmt "\n", ##__VA_ARGS__);
#define THREAD_TRC_LOG(fmt, ...)    printf("[THREAD-T]" fmt "\n", ##__VA_ARGS__);
...
/*!
 * @brief 无效线程句柄（使用 0 表示无效，兼容 pthread_t 为整数类型的实现）
 */
...
private:
    ...
    bool                            m_attr_inited;  /*!< 属性是否已初始化 */
    bool                            m_is_ok;        /*!< 互斥锁是否初始化成功 */

public:
    /**
     * @brief 构造函数
     * @param recursive 是否创建递归互斥锁，默认为 true
     */
...
public:
    ...
    /**
     * @enum thread_cancel_state_t
     * @brief 线程取消状态枚举
     */
    typedef enum _e_thread_cancel_state_
    {
        THREAD_CANCEL_DISABLE = 0,  /*!< 禁止取消 */
        THREAD_CANCEL_ENABLE,       /*!< 允许取消 */
    } thread_cancel_state_t;

    /**
     * @enum thread_cancel_type_t
     * @brief 线程取消类型枚举
     */
    typedef enum _e_thread_cancel_type_
    {
        THREAD_CANCEL_TYPE_DEFERRED = 0,   /*!< 延迟取消（到达取消点才取消） */
        THREAD_CANCEL_TYPE_ASYNC,          /*!< 异步取消（随时可能取消） */
...
#ifdef USE_ATOMIC_THREAD_STATE
    std::atomic<thread_state_t>     m_state;            /*!< 线程状态（原子类型） */
#else
    thread_state_t                  m_state;            /*!< 线程状态 */
#endif
    mutable mutex                   m_mutex;            /*!< 状态保护互斥锁（仍用于保护句柄和复合操作） */
